...
class Sales extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['customer_m', 'sale_m', 'item_m', 'table_m']);
        check_not_login();
    }
    ...
    public function xx()
    {
        ...
        $this->template->load('template', 'transaction/sales/sales_list', $data);
    }

    public function index()
    {
        $data['customer'] = $this->customer_m->get()->result();
        $data['table'] = $this->table_m->get()->result();
        $data['item'] = $this->item_m->get()->result();
        $data['cart'] = $this->sale_m->get_cart();
        $data['invoice'] = $this->sale_m->invoice_no();
    ...
    public function edit_chart()
    {
        ...
        $data['customer'] = $this->customer_m->get()->result();
        $data['item'] = $this->item_m->get()->result();
        $data['cart'] = $this->sale_m->get_cart2($invoice);
        //$data['invoice'] = $this->sale_m->invoice_no();
    ...
    public function process()
    {
        ...
        if (isset($_POST['add_cart'])) {
            ...
            $cek_cart = $this->sale_m->get_cart(['keranjang.item_id' => $item_id]);
        ...
        if (isset($_POST['process_payment'])) {
            ...
            $this->sale_m->update_stat_order($rowx);
    ...
    public function reset()
    {
        if (isset($_POST['cancel_payment'])) {
            ...
            $this->sale_m->reset_cart(['user_id' => $userid]);
